...
document.getElementById(
	'bestScore'
).innerHTML = `You've donated ${currentScore} minutes of school to children in Bali`
...
var deathAnimation = function () {
	if (splash) {
		...
		ctx.strokeStyle = '#fff'
		ctx.fillStyle = '#000'
		var txt1 = 'Your bird donated'
		var txt2 = 'minutes of school'
		var txt = score;
		ctx.strokeText(txt, (width - ctx.measureText(txt).width) / 2, height * 0.25)
		ctx.fillText(txt, (width - ctx.measureText(txt).width) / 2, height * 0.25)
		ctx.strokeText(txt1, (width - ctx.measureText(txt1).width) / 2, height * 0.18)
		ctx.fillText(txt1, (width - ctx.measureText(txt1).width) / 2, height * 0.18)
		ctx.strokeText(txt2, (width - ctx.measureText(txt2).width) / 2, height * 0.32)
		ctx.fillText(txt2, (width - ctx.measureText(txt2).width) / 2, height * 0.32)
		splash = undefined
	} else {
		ctx.drawImage(scoreBoard, width / 2 - 113, height / 2 - 54)
		ctx.font = '20px "Press Start 2P"'
		ctx.lineWidth = 5
		ctx.strokeStyle = '#fff'
		ctx.fillStyle = '#000'
...
var drawScore = function () {
	...
	ctx.strokeText(txt, (width - ctx.measureText(txt).width) / 2, height * 0.25)
	ctx.fillText(txt, (width - ctx.measureText(txt).width) / 2, height * 0.25)
	// ctx.strokeText(txt1, (width - ctx.measureText(txt1).width) / 2, height * 0.08)
	// ctx.fillText(txt1, (width - ctx.measureText(txt1).width) / 2, height * 0.08)
	// ctx.strokeText(txt2, (width - ctx.measureText(txt2).width) / 2, height * 0.22)
	// ctx.fillText(txt2, (width - ctx.measureText(txt2).width) / 2, height * 0.22)
}
